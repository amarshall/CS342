...
public class TaskManager extends Observable {
  ...
  public void filterInput(String line) {
    ...
    if(tab.equals("performance")) {
      Performance p = null;
      try {
        p = new Performance(Double.parseDouble(fields[0]), Double.parseDouble(fields[1]), Double.parseDouble(fields[2]), Double.parseDouble(fields[3]));
      } catch(NumberFormatException e) {
        System.err.println("Bad input.");
        System.exit(1);
      }
      performanceTab.update(p);
    }

    if(tab.equals("user")) {
      User u = null;
      try {
        u = new User(fields[0], fields[1]);
      } catch(NumberFormatException e) {
        System.err.println("Bad input.");
        System.exit(1);
      }
